...
const DEFAULT_PORT = Number(process.env.VISUAL_SNAPSHOT_PORT || 5184);
...
function cacheVersion(indexHtml) {
  ...
  if (unique.length !== 1 || !/^\d{8}-\d{2}$/.test(unique[0])) {
    throw new Error('Cache versions must match and use YYYYMMDD-NN before taking visual snapshots.');
  }
  return unique[0];
}
...
async function pruneOldSnapshots() {
  ...
  const versions = entries
    ...
    .sort();
  for (const version of versions.slice(0, Math.max(0, versions.length - 2))) {
...
async function writeManifest(outputDir, manifest) {
  ...
  const lines = [
    `# Visual Snapshot ${manifest.version}`,
    '',
  ...
  for (const shot of manifest.screenshots || []) {
    lines.push(`- ${shot.page}: \`${shot.file}\``);
  }
  if (manifest.error) {
...
async function run() {
  ...
  await mkdir(SNAPSHOT_ROOT, { recursive: true });
  const outputDir = resolve(SNAPSHOT_ROOT, version);
  ...
  try {
    ...
    async function shot(pageName, fileName) {
      const file = `${fileName}.png`;
      await settle();
      await page.screenshot({ path: resolve(outputDir, file), fullPage: true });
      manifest.screenshots.push({ page: pageName, file });
    }
    ...
    console.log(`Visual snapshots saved: ${outputDir}`);
  } catch (error) {
    manifest.ok = false;
    manifest.error = error?.message || String(error);
    await writeManifest(outputDir, manifest);
    console.error(`Visual snapshot blocked or failed: ${manifest.error}`);
    process.exitCode = 1;
  } finally {

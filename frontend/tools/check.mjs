...
assertCheck(jlptFeedItem?.sources?.length === 1 && jlptFeedItem.sources[0].url.includes('/topics/list2026.html') && !JSON.stringify(jlptFeedItem).includes('/application/'), 'JLPT reading material uses one article-specific official date source');
...
assertCheck(appJs.includes('materialDisplayTopic') && appJs.includes('materialSourceName') && appJs.includes('materialPrimarySourceLink') && appJs.includes('material-card-facts'), 'material cards keep one level tag, one fact line, and one primary source');
...
assertCheck(designMd.includes('学习标签、来源与卡片交互契约') && uiKitMd.includes('素材库共享组件规则'), 'design standards require shared badges, direct sources, aligned cards, and visual review');

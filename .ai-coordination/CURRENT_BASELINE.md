...
- 状态：`RELEASED / OBSERVE`
- 本轮模块化、PR、main 合并和 Production 发布均已完成。
- 不需要再次创建 Preview、再次合并 main 或再次发布 Production。
- 真实 Mac/iPhone Safari 可做一次精简 Production 抽查；仅在发现真实回归时重新进入修复流程。

## 接下来顺序

1. 更新并提交本次文档基线重置。
2. 可选：在 Production 做 Mac/iPhone Safari 精简抽查，并把结果记录到 `REAL_DEVICE_ACCEPTANCE_CHECKLIST.md`。
3. 稳定观察后删除已合并的 `refactor/app-js-vocab-round2` 本地与远端分支。
4. 新产品任务需要重新排期；当前建议候选为离线中文释义覆盖扩展的缺口统计与数据源设计。

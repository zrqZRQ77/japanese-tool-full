...
- 等级：P2
- 状态：FIXED（缓存 `20260712-02` 自动回归 PASS，等待实机复核）
...
- 处理结论：手机抽屉改为与桌面一致的单列图标导航和纯白背景；移除内部伪元素提示并补齐固定定位提示节点；手机 ruby 偏移使用当前设置值的 60%。录音功能保持不变。390/430/1280 回归 PASS，无横向溢出或控制台错误。

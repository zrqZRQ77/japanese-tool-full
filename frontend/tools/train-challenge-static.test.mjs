...
assert.match(html, /class="question-core"/);
...
assert.match(html, />重新挑战<\/button>/);
assert.match(html, />保存成绩<\/button>/);
assert.match(html, />分享成绩<\/button>/);
assert.doesNotMatch(html, /<small>PNG<\/small>/);
assert.doesNotMatch(html, /或复制链接<\/small>/);
...
assert.match(js, /const PUBLIC_CHALLENGE_DISPLAY_URL = 'yomeru\.japanese-hub\.com\/challenge\/train'/);
...
assert.match(js, /function drawYomeruMark/);
assert.match(js, /drawYomeruMark\(context, 58, 35, 0\.82\)/);
assert.match(js, /context\.fillText\(PUBLIC_CHALLENGE_DISPLAY_URL, 82, 1260\)/);

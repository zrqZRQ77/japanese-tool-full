...
assert.equal(report.corpus.jmdictFallbackHits, 96);
assert.equal(report.corpus.unresolvedAfterJmdict, 28);
assert.equal(report.corpus.combinedResolvablePercent, 86.5);
const exactEvidenceMisses = report.unresolvedCases.filter(item => item.expectedMeaningClass === 'jmdict-fallback');
assert.deepEqual(exactEvidenceMisses.map(item => item.id), ['LQ-179', 'LQ-181']);
assert.ok(report.unresolvedCases.filter(item => item.expectedMeaningClass !== 'jmdict-fallback').every(item => item.expectedMeaningClass === 'unknown'));
...
assert.equal(gap('一日', 'ついたち').jmdictAvailable, false);
assert.deepEqual(gap('人気', 'にんき').examples, ['LQ-180']);
assert.deepEqual(gap('人気', 'ひとけ').examples, ['LQ-181']);
assert.equal(gap('人気', 'ひとけ').jmdictAvailable, false);

...
function renderQuotaStatus(){
  ...
  quotaTarget.innerHTML = Object.keys(METERED_FEATURES).map(key=>{
    ...
    const value = status.paid
      ? `${status.used}/${status.limit} 次`
      : 'Pro 可用';
    return `<div class="quota-row"><span>${escapeHtml(status.label)}</span><b>${escapeHtml(value)}</b></div>`;
  }).join('');
}
...
let ACTIVE_PRACTICE_MODULE = 'typing';
...
function refreshRetellAdvice(){
  ...
  const retellLock = document.getElementById('retellModuleLock');
...
function updateReadingQuizFlowState(){
  ...
  start?.classList.toggle('is-hidden', !hasText || hasQuiz);
...
function filteredVocabForPage(){
  ...
  const jlptFilter = document.getElementById('vocabJlptFilter')?.value || 'all';
...
function vocabListMarkup(items){
  ...
  return items.map(v=>{
    ...
    const sourceTitle = vocabSourceLabel(v);
    const sourceMeta = sourceTitle
      ? `<span class="vocab-source-title">${escapeHtml(sourceTitle)}</span><span class="vocab-source-kind">${v.sourceTitle || v.source ? '阅读来源' : '本地生词'}</span>`
      : '<span class="vocab-source-title">手动添加</span><span class="vocab-source-kind">本地生词</span>';
    ...
    <li class="vocab-table-row">
      <div class="vocab-cell vocab-cell-word">
        <span class="vocab-word-main">${escapeHtml(v.word)}</span>
        ${v.level ? `<span class="vocab-level-chip level-${escapeHtml(String(v.level).toLowerCase())}">${escapeHtml(v.level)}</span>` : ''}
      </div>
      <div class="vocab-cell vocab-cell-meaning" title="${escapeHtml(meaning)}">
        <span class="vocab-reading">${escapeHtml(v.reading || '读音待补充')}</span>
        <span class="vocab-meaning${meaningTone}">${escapeHtml(meaning)}</span>
        <span class="vocab-source-inline">${sourceMeta}</span>
      </div>
      <div class="vocab-cell vocab-cell-side">
        <div class="vocab-cell-mastery">
          <span class="vocab-mastery ${mastery.tone}"><i aria-hidden="true"></i>${mastery.label}</span>
          <small>${isDueNow ? '现在可复习' : formatDue(v.dueAt)}</small>
        </div>
...
function vocabMasteryLabel(v, isDueNow){
  ...
  return {
    ...
    unsure:{label:isDueNow && !v.lastPracticeRating ? '待复习' : '有点犹豫', tone:'unsure', icon:'~'},
...
function renderVocab(){
  ...
  const now = Date.now();
  const weakCount = vocabData.filter(v=>['again', 'hard'].includes(v?.lastPracticeRating || '')).length;
...
function renderCard(){
  ...
  updateFlashProgress();
  area.innerHTML = `
    <div class="flash-stage ${cardFlipped ? 'is-flipped' : ''}" role="button" tabindex="0" aria-label="${cardFlipped ? '已显示' : '查看'} ${escapeHtml(v.word)} 的读音和释义" onclick="flipCard()" onkeydown="handleFlashCardKey(event)">
      <div class="flash-card-inner">
        <div class="flash-face flash-front">
          <div class="flash-word">${escapeHtml(v.word)}</div>
          <div class="flash-tap-hint">点击卡片翻转查看释义</div>
        </div>
        <div class="flash-face flash-back">
          <div class="flash-word">${escapeHtml(v.word)}</div>
          <div class="flash-reading">${escapeHtml(v.reading || '读音待补充')}</div>
          <div class="flash-meaning">${escapeHtml(displayVocabMeaning(v.meaning))}</div>
        </div>
      </div>
    </div>
    <div class="rate-row ${cardFlipped ? 'show' : ''}">
...
function rateCard(rating){
  ...
  if(v){
    ...
    if(rating === 'again'){
      ...
      v.repetition = Math.max(0, currentRepetition); // 维持当前等级,不前进
    } else { // easy
...
function renderSourceDirectory(){
  ...
  const sources = READING_SOURCES
    .filter(source => sourceLevelMatches(ACTIVE_SOURCE_LEVEL, source.level))
    .filter(source => sourceTypeMatches(ACTIVE_SOURCE_TYPE, source));
  target.innerHTML = sources.length ? sources.map(source => `
    <article class="source-directory-item ${recommendedSourceForLevel(source, recommended) ? 'recommended' : ''}">
      <button class="source-directory-icon" onclick="openRecommendedSource('${source.url}')" aria-label="打开 ${escapeHtml(source.title)}">
        ${sourceCategoryIconSvg(source)}
      </button>
      <div class="source-directory-copy">
        ...
        <div class="source-traits">
          ...
          ${(source.traits || []).map(trait => `<span>${escapeHtml(trait)}</span>`).join('')}
        </div>
      </div>
      <div class="btnrow">
        <button class="btn-primary" onclick="useSourceUrl('${encodeURIComponent(source.url)}')">导入并阅读</button>
        <button class="btn-ghost source-external-action" onclick="openRecommendedSource('${source.url}')">打开网站 ${sourceIconSvg()}</button>
      </div>
    </article>
